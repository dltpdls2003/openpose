...
url = 'http://192.168.1.105:8080/behavior/add-behavior'
count = 0
...
def output_keypoints_with_lines(POSE_PAIRS, frame):
    ...
    if (points[1] is not None) and (points[8] is not None):
        calculate_degree(point_1=points[12], point_2=points[13],
                         point_3=points[9], point_4=points[10], frame=frame_line)
...
def calculate_degree(point_1, point_2, point_3, point_4, frame):
    ...
    # 이상행동 저장코드
    if abs(deg - deg2) >= 90:
        shutil.copy(everyFile, file_name_path)
        #cv2.imwrite(file_name_path, frame2)  # file_name_path에 캡쳐 저장
        string = "Climb"
        ...
        cctv_name = 'kimcom'
        timestr = time.strftime("%Y%m%d%H%M%S")
        obj = {'bh_id': 0, 'cctv_name': cctv_name, 'datetime': timestr}
        image = {'image': open('captures/test' + str(now) + '.jpg', 'rb')}

        try:
            response = requests.post(url, data=obj, files=image)
...
while cap.isOpened():
    ...
    file_name_path = 'captures/test' + str(now) + '.jpg'    # climb
    everyFile = 'allCaptures/test' + str(now) + '.jpg'
    cv2.imwrite(everyFile, frame)  # file_name_path에 캡쳐 저장

    # path_list에 캡쳐 사진 이름 저장
    path_list.append(everyFile)
    # path_list 개수 세기
    path_list_len = len(path_list)
    list_num = path_list_len - 1
    try:
        frame_man = output_keypoints(image_path=path_list[list_num], proto_file=protoFile_body_25,
                                 weights_file=weightsFile_body_25,
                                 threshold=0.1, model_name=path_list[list_num].split('\\')[-1],
                                 BODY_PARTS=BODY_PARTS_BODY_25)
    except TypeError:
        print("TypeError!")
    output_keypoints_with_lines(POSE_PAIRS=POSE_PAIRS_BODY_25, frame=frame_man)

    # while문 100번 돌아가면 pathlist배열초기화, allCaptures내사진삭제
    count += 1

    if count >= 100:
        for i in range(0, 100):
            del path_list[i]
            os.remove('allCaptures/test' + str(i))
            os.remove('allCaptures/test' + str(i))
        count = 0
